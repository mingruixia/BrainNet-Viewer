...
[pth,nam,ext,n1] = BrainNet_spm_fileparts(deblank(p));
...
switch ext,
    ...
    case {'.img','.IMG'},
        if ~BrainNet_spm_existfile(fullfile(pth,[nam '.hdr'])) && ~BrainNet_spm_existfile(fullfile(pth,[nam '.HDR'])),
...
if isempty(n) && length(V.private.dat.dim) > 3
    ...
    for i = 2:V.private.dat.dim(4)
        V0(i) = BrainNet_spm_vol_nifti(p, i);
    end
    V = V0;
end
